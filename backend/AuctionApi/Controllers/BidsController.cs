...
namespace AuctionApi.Controllers
{
    ...
    public class BidsController : ControllerBase
    {
        ...
        public async Task<ActionResult<Bid>> PlaceBid([FromBody] BidDto bidDto)
        {
            ...
            if (item.BidStartTime.HasValue && DateTime.UtcNow < item.BidStartTime.Value)
            {
                return BadRequest("Bidding has not started yet.");
            }

            if (item.BidEndTime.HasValue && DateTime.UtcNow > item.BidEndTime.Value)
            {
                return BadRequest("Bidding has ended.");
            }
